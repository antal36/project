"""
Fruit image classification training script using MobileNetV2.

This module loads image data, applies preprocessing and augmentation,
trains a transfer-learning model based on MobileNetV2, evaluates it on a
test set, and produces multiple visualizations of performance.

The final trained model is saved to disk.
"""
...
def train_model(
    ...
    """
    Build, compile, and train a transfer-learning image classification model.

    A MobileNetV2 backbone pretrained on ImageNet is used as a frozen
    feature extractor. Custom fully connected layers are added on top
    for classification.
    ...
    ----------
    train_gen : KerasIterator
        Data generator yielding augmented training images and labels.
    val_gen : KerasIterator
        Data generator yielding validation images and labels.
    steps_per_epoch : int
...
def main() -> None:
    ...
    6. Train the model using transfer learning.
    7. Evaluate performance on the test set.
    8. Save the trained model and generate visual reports.

    Returns
    -------
    None
    """
    images, labels, names = load_data(data_type="Category")
    ...
    class_names: list[str] = ["apple", "banana", "cherry", "chickoo", "grapes",
    ...
    plot_model(model=model, to_file="model_classifier.png")
